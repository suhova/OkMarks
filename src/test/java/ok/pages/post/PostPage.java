...
public class PostPage extends BasePage {
    ...
    private final By ADD_POST = By.xpath(".//*[contains(@href,'/post')]");
    ...
    public PostPage addPost(String message) {
        ...
        Assert.assertTrue("Не найдено поля написать заметку",
                explicitWait(ExpectedConditions.visibilityOfElementLocated(WRITE), 10, 500));
        driver.findElement(WRITE).sendKeys(message);
        Assert.assertTrue("Не найдено кнопки добавления заметки",
                explicitWait(ExpectedConditions.visibilityOfElementLocated(SUBMIT), 10, 500));
